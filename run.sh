...
while /bin/true; do
	...
	for scan in indexscan bitmapscan; do

		#for eic in 0 16 128; do
		for eic in 0 1 16 64; do

			for pg in master prefetch-simple prefetch-complex; do
				...
				for iomethod in worker io_uring sync; do
					...
					for ioworkers in 3 12; do
						...
						echo $cnt $scan $eic $pg $iomethod $ioworkers >> test.csv
...
for shared_buffers in 32GB 4GB; do
	...
	for dataset in uniform linear cyclic linear_1 cyclic_1 linear_10 cyclic_10 linear_25 cyclic_25; do
		...
		while IFS= read -r line; do
			...
			echo "cnt=$cnt dataset=$dataset scan=$scan eic=$eic pg=$pg iomethod=$iomethod ioworkers=$ioworkers shared_buffers=$shared_buffers"
			...
			# enable prefetch on the "simple" patch
			if [ "$pg" == "prefetch-simple" ]; then
				prefetch="SET enable_indexscan_prefetch = on;"
			fi

			# set the path to the correct postgres version
			PATH=$BUILDS/$pg/bin:$PATH_OLD

			pg_ctl -D $DATADIR -l pg-$pg.log start
...
EOF
			...
			echo $id $cnt $dataset $scan $eic $r $pg $iomethod $ioworkers $shared_buffers $start $end $optimal $tuples_total $tuples $pages_total $pages $cost $time_uncached $time_cached >> results.csv

			pg_ctl -D $DATADIR -l pg-$pg.log stop
